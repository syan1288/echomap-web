...
/** React 18 Strict Mode 会双挂载，ScrollTrigger.onEnter 会跑两次；triggerOnce 时第二次直接落静态终态 */
const scrollEnterOnceGuard = new Set<string>();
...
/** 开源 gsap 包不含 Club 版 SplitText，改为手动按字拆分 */
function splitTextToChars(el: HTMLElement, content: string): { chars: HTMLElement[]; revert: () => void } {
  el.textContent = '';
  ...
  useGSAP(
    () => {
      ...
      const build = () => {
        ...
        const split = splitTextToChars(el, text);
      ...
      const inners = () =>
        wrappersRef.current.map((w) => w.firstElementChild).filter(Boolean) as HTMLElement[];
      ...
      const play = () => {
        ...
        const tl = gsap.timeline({
          ...
          onRepeat: () => {
            ...
            if (isVertical) {
              gsap.set(strips, {
                y: (i, t) => parseFloat((t as HTMLElement).getAttribute('data-start-y') || '0'),
              });
            } else {
              gsap.set(strips, {
      ...
      const onScrollEnter = () => {
        const guardKey = `${text}::${start}`;
        if (triggerOnce && scrollEnterOnceGuard.has(guardKey)) {
          build();
          if (scrambleCharset) randomizeScrambles();
          cleanupToStill();
          setReady(true);
        ...
        if (triggerOnce) scrollEnterOnceGuard.add(guardKey);
        create();
      };
